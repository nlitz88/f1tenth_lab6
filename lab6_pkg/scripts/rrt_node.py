...
class RRT(Node):
    ...
    def __costmap_callback(self, costmap: OccupancyGrid) -> None:
        ...
        try:
            rrt_path = rrt(costmap=numpy_occupancy_grid,
        ...
        new_path = Path()
        # First, begin constructing new path object. 
        new_path.header.frame_id = self.__path_frame
        new_path.header.stamp = costmap.header.stamp
        # Before we can add poses to our path, we need the from the
        # occupancy grid's parent frame to the destination frame so that the
        # path's poses can be in terms of the desired path_frame.
        grid_parent_to_path_frame_transform = self.__transform_buffer.lookup_transform(target_frame=self.__path_frame,
                                                                                        source_frame=costmap.header.frame_id,
                                                                                        time=Time())
        # For each position in the grid path returned by RRT, project the
        # grid position back out into continuous space within the parent
        # frame of the occupancy grid, and then transform that position to
        # the desired path frame.
        for grid_position in rrt_path:
            ...
            transformed_pose = tf2_geometry_msgs.do_transform_pose_stamped(pose=new_pose,
                                                                            transform=grid_parent_to_path_frame_transform)
            new_path.poses.append(transformed_pose)
        # Finally, publish the transformed path.
        self.__path_publisher.publish(new_path)
