...
class RRT(Node):
    ...
    def __costmap_callback(self, costmap: OccupancyGrid) -> None:
        ...
        new_path = Path()
        # If the path frame is not specified, just publish the path (and the
        # poses within it) in the same frame as the costmap we receive.
        # TODO: Just update to transform to whatever frame is provided??
        ...
        # # As a quick debugging step, I can just plot these on the "debugging
        # # costmap?"
        # # self.get_logger().info(f"Temporarily visualizing path generated by RRT.")
        # for position in rrt_path:
        #     x, y = position
        #     numpy_occupancy_grid[y, x] = 100
